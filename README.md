...
> 上图：在 code-server 里，光标在代码编辑器/终端时自动用英文，点到 CodeBuddy AI 侧边栏自动切中文，全程不用碰输入法切换键。

## ✨ 与众不同之处：它能看进应用内部

市面上的输入法自动切换工具（包括知名的 Input Source Pro 等）大多只能做到**按应用**或**按网址**切换。InputSwitcher 更进一步——它能**分辨同一个应用内部的不同区域**，这是同类产品几乎都做不到的：

- 🖥️ **VSCode 代码编辑器** → 英文，但**右侧 AI 助手侧边栏**（CodeBuddy / Copilot Chat）→ 中文
- ⌨️ **同一个终端**里：跑 `claude`（Claude Code）时自动切**中文**，敲普通命令时自动切**英文**
- 🌐 **桌面版 VSCode 和浏览器里的网页版 [code-server](https://github.com/coder/code-server) 同一套规则都生效**——因为它识别的是控件本身的特征，而非应用或网址

换句话说：你和 AI 用中文对话、写代码用英文，**全程不用碰一下输入法切换键**，哪怕它们在同一个窗口里。
...
- **按应用内区域**：靠网页/Electron 控件的 DOM class 识别，例如：
  - VSCode / code-server 的**代码编辑器** → 英文
  - VSCode 的**集成终端** → 英文
  - CodeBuddy / Copilot Chat **AI 侧边栏** → 中文
- **按窗口标题**：例如系统「终端」里跑 Claude Code 时 → 中文，普通命令行 → 英文
- **按运行的命令**：VSCode 终端里跑 `claude` 时 → 中文（靠辅助功能描述识别，普通命令则保持英文）
...
默认规则里的中文目标是**系统自带的简体拼音**（人人都有、最稳定）。如果你想用第三方输入法，在「规则设置」里把对应规则的「切换到」改成你的输入法即可。

### 中文输入法兼容性（实测）

这个工具靠系统 API（`TISSelectInputSource`）切换输入法。不同输入法对"被程序切换"的响应质量不一样，实测结论：

| 输入法 | 兼容性 | 说明 |
|--------|--------|------|
| **系统简体拼音** | ✅ 完美 | 原生 IME，切过去立即就绪，最稳定，默认即用 |
| **微信输入法** | ✅ 良好 | 实测响应正常，可放心使用 |
| **豆包输入法** | ⚠️ 不推荐 | 被切换 API 选中后引擎加载慢，常出现"菜单栏图标已切到豆包、但实际仍打出英文"。这是豆包自身对程序化切换的响应问题，本工具无法绕过 |

> 建议：中文场景优先用**系统简体拼音**或**微信输入法**。如果你重度依赖豆包且遇到上述问题，目前没有可靠解法。
